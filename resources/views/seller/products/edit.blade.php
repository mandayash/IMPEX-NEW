    ...
    <div class="product-form-card">
        <form method="POST" action="{{ route('seller.products.update', $product->id) }}" enctype="multipart/form-data">
            ...
            <div class="alert alert-info">
                Form akan di-submit ke: {{ route('seller.products.update', $product->id) }}
            </div>
            ...
            <div class="form-section">
                ...
                <div class="photo-upload-area" onclick="document.getElementById('image').click()">
                    ...
                    <div class="photo-upload-text">Change Photo 1:1</div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror"
                       id="name" name="name" value="{{ old('name', $product->name) }}" maxlength="255"
                       placeholder="Type Here!" required>
                <div class="char-count"><span id="nameCount">{{ strlen(old('name', $product->name)) }}</span>/255</div>
                @error('name')
            ...
            <div class="form-section">
                ...
                <div class="char-count"><span id="descCount">{{ strlen(old('description', $product->description)) }}</span>/3000</div>
                @error('description')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
                ...
                <div class="commodity-grid">
                    ...
                    <div class="commodity-option">
                        <input type="radio"
                            ...
                            value="{{ $type }}"
                            id="{{ Str::slug($type) }}"
                            {{ old('commodity_type', $product->commodity_type) == $type ? 'checked' : '' }}
                            required>
            ...
            <div class="form-section">
                ...
                <div class="char-count"><span id="priceCount">{{ strlen(old('price', $product->price)) }}</span>/12</div>
                @error('price')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
